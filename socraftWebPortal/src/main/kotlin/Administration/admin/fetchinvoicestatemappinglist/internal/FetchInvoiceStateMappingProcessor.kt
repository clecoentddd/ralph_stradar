package administration.admin.fetchinvoicestatemappinglist.internal

import administration.admin.domain.commands.fetchinvoicestatemappinglist.MarkListOfInvoiceStateFetchedCommand
import administration.admin.fetchinvoicestatemappinglist.internal.adapter.FetchBoondAPIInvoiceStateMapping
...
class FetchInvoiceStateMappingProcessor(
  ...
  private fun fetchAndDispatch(event: InvoiceStateMappingUpdateRequestedEvent) {
    ...
    val mappedPayload =
            adapterResult.states.map { item -> InvoiceState(code = item.code, label = item.label) }
