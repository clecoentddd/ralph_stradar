package administration.client.fetchorders.internal.adapter
...
data class OrdersListInfo(val orders: List<ListOfOrdersItem>)
...
class FetchBoondAPIOrderList(
        ...
        fun fetch(companyId: Long, projectReference: String? = null): OrdersListInfo {
                logger.info(
                        "Fetching orders for companyId: $companyId, projectReference: $projectReference"
                )
                ...
                val urlBuilder =
                        UriComponentsBuilder.fromUriString("$baseUrl/orders")
                                ...
                                .queryParam(
                                        "fields[orders]",
                                        "reference,title,state,orderDate,startDate,endDate,informations"
                                )
                ...
                return try {
                        val response =
                                restTemplate.exchange(
                                        ...
                                        BoondOrdersResponse::class.java
                                )

                        logger.info(
                                "Response received. Total orders in response: ${response.body?.data?.size ?: 0}"
                        )
                        ...
                        val filteredOrders =
                                allOrders.filter { order ->
                                        val matchesCompany =
                                                order.relationships.company?.data?.id == companyId
                                        val matchesProject =
                                        ...
                                        ListOfOrdersItem(
                                                ...
                                                totalIncludingTaxes =
                                                        it.attributes
                                                                ...
                                                                ?.totalIncludingTaxes
                                                                ?: 0.0,
                                                totalVat = it.attributes.informations?.totalVat
                                                                ?: 0.0
                                        )
                                }

                        logger.info(
                                "Successfully fetched ${orders.size} orders for companyId: $companyId"
                        )
