...
class FetchBoondOrderListProcessor(
  ...
  private fun fetchAndDispatch(event: ListOfProjectsFetchedEvent) {
    ...
    logger.info { "Dispatching MarkOrdersFetchedCommand with ${adapterResult.orders.size} items" }
    ...
    commandGateway
        ...
        .exceptionally { throwable ->
          logger.error(throwable) { "FAILED to process FetchBoondOrderList: ${throwable.message}" }
          null
        }
  }
}
