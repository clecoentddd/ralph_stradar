...
import administration.support.metadata.AppSecurityHeaders // Ensure this is imported
import java.util.*
...
class AdminConnectedCheckingconnectionIdisnotnullReadModelTest : BaseIntegrationTest() {
        ...
        fun `Admin Connected Read Model Test`() {
                ...
                val toConnectCommand =
                        ToConnectCommand(connectionId = connectionId, email = testEmail)

                // 1. Prepare the MetaData the interceptor is looking for
                val testMetaData =
                        MetaData.with(AppSecurityHeaders.SESSION_ID_HEADER, "test-session-id")
                                .and(AppSecurityHeaders.COMPANY_ID_HEADER, "test-company-id")
