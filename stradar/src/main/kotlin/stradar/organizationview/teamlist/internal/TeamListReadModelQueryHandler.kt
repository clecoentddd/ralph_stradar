...
class TeamListReadModelQueryHandler(private val repository: TeamListReadModelRepository) {
  ...
  /**
   * Checks if a team name is already taken within the organization. Returns true if the name is
   * UNIQUE (available), false if it exists.
   */
  @QueryHandler
  fun handle(query: TeamNameAlreadyExistsQuery, metadata: MetaData): Boolean {
    ...
    val exists = repository.existsByNameAndOrganizationId(query.teamName, organizationId)
  ...
  /**
   * Fetches all teams scoped to the organization. Enforces organizationId from MetaData to prevent
  ...
  /**
   * Fetches the name of a specific team. Also enforces an organization matching check to ensure the
   * team belongs to the requester's org.
   */
  @QueryHandler
