...
class InitiativesReadModelQueryHandler(private val repository: InitiativesReadModelRepository) {
        ...
        /**
         * * Handles the high-level Organization Dashboard lookup. Groups initiatives by Team and
         * then by Level (Step).
         */
        @QueryHandler
        fun handle(query: AllInitiativesForOrganizationQuery): OrganizationInitiativeListResponse {
                // 1. Fetch the data
                val results =
                        repository.findAllByOrganizationId(query.organizationId).filter {
                                it.statut != "Deleted"
                        }
                ...
                val entity =
                        repository.findById(query.initiativeId).orElseThrow {
                                NoSuchElementException(
                                        "Initiative not found: ${query.initiativeId}"
                                )
                        }
