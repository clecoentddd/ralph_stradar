...
class EnvironmentalChangesQueryHandler(
  ...
  fun handle(
    ...
    val entities =
        repository.findByEnvironmentalChangeIdAndOrganizationId(
            query.environmentalChangeId, organizationId)
  ...
  fun handle(
    ...
    logger.info { "QUERY: Fetching Team List for Team [${query.teamId}] (Org: $organizationId)" }

    val entities = repository.findByTeamIdAndOrganizationId(query.teamId, organizationId)
    ...
    return EnvironmentalChangesReadModel(
        environmentalChangeId = entities.firstOrNull()?.environmentalChangeId ?: UUID.randomUUID(),
        teamId = query.teamId,
        organizationId = organizationId,
        elements = entities.map { it.toDTO() })
  }
